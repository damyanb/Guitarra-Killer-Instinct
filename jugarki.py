...
import time
import keyboard  # Importar el módulo keyboard
...
def callback(indata, frames, time, status):
    ...
    if not hasattr(callback, "key_pressed"):
        callback.key_pressed = False  # Variable para mantener el estado de la tecla
    ...
    if any(indata):
        ...
        # Comprobar si la nota más cercana es un C
        if closest_note == "C3" and not callback.key_pressed:
            # Realizar la acción deseada, por ejemplo, emular la pulsación de la tecla A
            keyboard.press('a')
            callback.key_pressed = True

        if closest_note != "C3" and callback.key_pressed:
            # Liberar la tecla si la nota ya no es C
            keyboard.release('a')
            callback.key_pressed = False
        if closest_note == "G3" and not callback.key_pressed:
            # Realizar la acción deseada, por ejemplo, emular el doble clic de la tecla 'd'
            keyboard.press('d')
            callback.key_pressed = True
            sd.sleep(int(50))  # Esperar un breve momento
            keyboard.release('d')
            callback.key_pressed = False
            sd.sleep(int(50))  # Esperar un breve momento
            keyboard.press('d')
        ...
        if closest_note != "G3" and callback.key_pressed:
            # Liberar la tecla si la nota ya no es C
            keyboard.release('d')
            callback.key_pressed = False
        if closest_note == "D3" and not callback.key_pressed:
            # Realizar la acción deseada, por ejemplo, emular la pulsación de la tecla A
          keyboard.press('q')
          callback.key_pressed = True
          sd.sleep(int(50))  # Esperar un breve momento
        ...
        if closest_note != "D3" and callback.key_pressed:
            # Liberar la tecla si la nota ya no es C
            keyboard.release('q')
            callback.key_pressed = False
        if closest_note == "E3" and not callback.key_pressed:
            # Realizar la acción deseada, por ejemplo, emular la pulsación de la tecla A
            keyboard.press('j')
            callback.key_pressed = True

        if closest_note != "E3" and callback.key_pressed:
            # Liberar la tecla si la nota ya no es C
            keyboard.release('j')
            callback.key_pressed = False
        if closest_note == "F3":
...
try:
    ...
    with sd.InputStream(channels=1, callback=callback, blocksize=WINDOW_STEP, samplerate=SAMPLE_FREQ):
        while True:
            sd.sleep(int(1000 * WINDOW_STEP / SAMPLE_FREQ))  # Ajuste del tiempo de espera
